...
def on_press(event):
    global thresh_value, dil_ero_value_x, dil_ero_value_y, block_size
    if event.key == 'j':
        thresh_value -= 5
    if event.key == 'k':
        thresh_value += 5
...
def show():
    ...
    thresh = cv.adaptiveThreshold(gray_equ, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, block_size,
                                  thresh_value)
    ...
    axes[1][0].set_title('blocksize = ' + str(block_size) + '. ' + 'C = ' + str(thresh_value))
